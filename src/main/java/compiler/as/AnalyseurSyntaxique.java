...
import compiler.al.UniteLexicale;
import compiler.asem.Symbole;
...
public class AnalyseurSyntaxique {
    ...
    public static String uniteCourante;

    public static AnalyseurLexical analyseurLexical;

    public static List<Symbole> tableSymbole = new ArrayList<Symbole>();

    public static int nbrInstructionCible;

    public static String adresse = "0";

    static List<Variable> tabledesident = new ArrayList<Variable>();

    // DECLARATION DE LA GENERATION
    public static List<InstruCible> vic = new ArrayList<InstruCible>();

    public static List<Variable> table_var = new ArrayList<Variable>();

    // L'etiquete c'est le nombre
    public static List<Integer> table_etiq = new ArrayList<Integer>();

    public AnalyseurSyntaxique() throws FileNotFoundException {

        analyseurLexical = new AnalyseurLexical("src/main/resources/test.txt");
    }
    ...
    public boolean isOperande() throws SemantiqueExecption {
        ...
        if (UniteLexicale.ident.equals(uniteCourante)) {
            if (contains(analyseurLexical.UNITECOURANT) != null) {
                genererInstr("load",
                        (tabledesident.get(((Integer) contains(analyseurLexical.UNITECOURANT)))).getAdresse());
            }
            return variableMustBeDeclared(AnalyseurLexical.UNITECOURANT);

        } else if (UniteLexicale.chaine.equals(uniteCourante) || UniteLexicale.entier.equals(uniteCourante)
    ...
    void verifieTypeForITOEAA(String typeIdent) throws SemantiqueExecption {
        ...
        if (type == null) {
            if (!Objects.equals(getCorrespondance(typeIdent), uniteCourante))
                throw new SemantiqueExecption("type erronÃ© !!");
        } else if (!typeIdent.equals(type))
            throw new SemantiqueExecption("type erronÃ© !!");
    }

    String mp = "";

    boolean isTermeOrIsExpressionArithmetiqueInAffectation(String typeIdent)
            throws SyntaxeException, SemantiqueExecption {
        if (isOperande()) {
    ...
    boolean declarationVariable() throws SyntaxeException, SemantiqueExecption {
        ...
        if (isTypeVariable()) {
            ...
            if (uniteCourante.equals(UniteLexicale.ident)) {
                ...
                if (contains(AnalyseurLexical.UNITECOURANT) == null) {
                    tabledesident.add(
                            new Variable(typeVariable, AnalyseurLexical.UNITECOURANT, this.adresse, "", "" + false));
                    // methode_variable();
                ...
                tableSymbole.add(s);
                uniteCourante = analyseurLexical.uniteSuivante();
                if (uniteCourante.equals(UniteLexicale.affec)) { // System.out.println("nnn"+uniteCourante);
                    uniteCourante = analyseurLexical.uniteSuivante();
                    // System.out.println("pp"+analyseurLexical.UNITECOURANT);
                    t = (Integer) contains(k);
                    // genererInstr("store",(tabledesident.get((int)t)).getAdresse());
                    genererInstr("loadc", analyseurLexical.UNITECOURANT);
                    genererInstr("store", (tabledesident.get(((Integer) contains(k)))).getAdresse());
                    tabledesident.get((Integer) t).modifiervaleur(analyseurLexical.UNITECOURANT);
                ...
                while (uniteCourante.equals(UniteLexicale.virgule)) {
                    ...
                    tableSymbole.add(s);
                    if (contains(AnalyseurLexical.UNITECOURANT) == null) {
                        tabledesident.add(new Variable(typeVariable, AnalyseurLexical.UNITECOURANT, this.adresse, "",
                                "" + false));
                        // methode_variable();
                        k = AnalyseurLexical.UNITECOURANT;
                        this.adresse += 1;
    ...
    boolean declarationConstante() throws SyntaxeException, SemantiqueExecption {
        ...
        if (uniteCourante.equals(UniteLexicale.constante)) {
            ...
            if (isTypeVariable()) {
                ...
                if (uniteCourante.equals(UniteLexicale.ident)) {
                    ...
                    if (contains(AnalyseurLexical.UNITECOURANT) == null) {
                        tabledesident.add(
                                new Variable(typeVariable, AnalyseurLexical.UNITECOURANT, this.adresse, "", "true"));
                        // methode_variable();
                        k = AnalyseurLexical.UNITECOURANT;
                        this.adresse += 1;
                    }
                    ...
                    if (uniteCourante.equals(UniteLexicale.affec)) {
                        ...
                        // System.out.println(""+analyseurLexical.UNITECOURANT);
                        tabledesident.get((Integer) t).modifiervaleur(analyseurLexical.UNITECOURANT);
                        tabledesident.get((Integer) t).setbolean("true");
         ...
         * if (!uniteCourante.equals(UniteLexicale.ident)) {
         * throw new
         * SyntaxeException("Declaration de constante non valide: 'ident' est required"
         * );
         * }
         * uniteCourante = analyseurLexical.uniteSuivante();
         * if (!uniteCourante.equals(UniteLexicale.affec)) {
         * throw new
         * SyntaxeException("Declaration de constante  non valide: '=' est required");
         * }
         * uniteCourante = analyseurLexical.uniteSuivante();
         * if (!isTerme()) {
         * throw new SyntaxeException("Declaration de constante non valide: " +
     ...
     * throw new GrammaireException("la variable: \'" + lexeme +
     * "\' doit être declarer. Ligne: " + AnalyseurLexical.lineNumber);
     * if(descVariable.nature != null)
     * throw new GrammaireException("la variable: \'" + lexeme +
     * "\' est une constante, affectation non possible. Ligne: " +
     * AnalyseurLexical.lineNumber);
     * uniteCourante = AnalyseurLexical.uniteSuivante();
     * if(!reconnaitreTerminal(CategorieLexical.AFFECT))
     * throw new GrammaireException("instruction d'affectation non valide . Ligne: "
     * + AnalyseurLexical.lineNumber);
     * uniteCourante = AnalyseurLexical.uniteSuivante();
     * if(exprA()) {​​​​​​​
     ...
     * }​​​​​​​
     * else throw new
     * GrammaireException("instruction d'affectation non valide . Ligne: " +
     * AnalyseurLexical.lineNumber);
     * }​​​​​​​
     * else if (uniteCourante == CategorieLexical.CMTLIGNE || uniteCourante ==
     * CategorieLexical.CMLMULTILIGNE){​​​​​​​
     * }​​​​​​​
     * else
     * {​​​​​​​
     * System.out.println(uniteCourante);
     * throw new GrammaireException("erreur synthaxique a la ligne: " +
     * AnalyseurLexical.lineNumber);
     * }​​​​​​​}
     */
    boolean affectation() throws SyntaxeException, SemantiqueExecption {
        if (uniteCourante.equals(UniteLexicale.ident)) {
            ...
            if (uniteCourante.equals(UniteLexicale.finLigne)) { // tabledesident.get(((int)contains(type)))).getAdresse()
                                                                // System.out.print("opiun"+l);
                genererInstr("store", (tabledesident.get(((Integer) contains(l)))).getAdresse());
                // genererInstr("load", );
                return true;
     ...
     * if(!exprA())
     * throw new GrammaireException("condition non valide . Ligne: " +
     * AnalyseurLexical.lineNumber);
     * if(opC())
     * {
     * opL1 = uniteCourante;
     * uniteCourante = AnalyseurLexical.uniteSuivante();
     * }
     * else throw new GrammaireException("condition non valide . Ligne: " +
     * AnalyseurLexical.lineNumber);
     * if(!exprA())
     * throw new GrammaireException("condition non valide . Ligne: " +
     * AnalyseurLexical.lineNumber);
    ...
    public void convertir() {
        for (InstruCible cible : vic) {
            if (cible.getOperateur().equals("plus")) {
                cible.setOperateur("add");
            }
            if (cible.getOperateur().equals("plus")) {
                cible.setOperateur("add");
            }
            if (cible.getOperateur().equals("moins")) {
                cible.setOperateur("sub");
            }
            if (cible.getOperateur().equals("prod")) {
                cible.setOperateur("mul");
            }
            if (cible.getOperateur() == "div") {
                cible.setOperateur("div");
            } // infEgal
            if (cible.getOperateur().equals("sup")) {
                cible.setOperateur("sup");
            }
            if (cible.getOperateur().equals("inf")) {
                cible.setOperateur("inf");
            }
            if (cible.getOperateur().equals("egal")) {
                cible.setOperateur("equal");
            }
            ///
            if (cible.getOperateur().equals("infEgal")) {
                cible.setOperateur("infe");
            }
            if (cible.getOperateur().equals("supEgal")) {
                cible.setOperateur("supe");
            }
            if (cible.getOperateur().equals("mod")) {
                cible.setOperateur("mod");
            }
            if (cible.getOperateur().equals("et")) {
                cible.setOperateur("and");
            }
            if (cible.getOperateur().equals("ou")) {
                cible.setOperateur("or");
            }
            if (cible.getOperateur().equals("ou")) {
                cible.setOperateur("or");
            }
    ...
    boolean ecriture() throws SyntaxeException, SemantiqueExecption {
        ...
        if (uniteCourante.equals(UniteLexicale.chaine)) {
            ...
            isExpressionArithemetique();
            genererInstr("store", (tabledesident.get(((Integer) contains("ntrevar")))).getAdresse());
            if (!uniteCourante.equals(UniteLexicale.parentFerme))
                throw new SyntaxeException("Erreur de syntaxe: ')' attendu");
            contains(lexeme);
            if (contains(lexeme) != null)
                genererInstr("write", (tabledesident.get(((Integer) contains(lexeme)))).getAdresse());
            else
                genererInstr("write", (tabledesident.get(((Integer) contains("ntrevar")))).getAdresse());
        }
        uniteCourante = analyseurLexical.uniteSuivante();
        if (!uniteCourante.equals(UniteLexicale.finLigne))
            throw new SyntaxeException("Erreur de syntaxe: ';' attendu");
    ...
    boolean lire() throws SyntaxeException {
        ...
        if (!uniteCourante.equals(UniteLexicale.finLigne))
            throw new SyntaxeException("Erreur de syntaxe: ';' attendu");
        genererInstr("read", "" + (tabledesident.get(((Integer) contains(lexeme)))).getAdresse());
        return true;
    }
    ...
    public void instruction() throws SyntaxeException, SemantiqueExecption {
        ...
        } else if (uniteCourante.equals(UniteLexicale.constante)) {
            declarationConstante();
        }

        else if (uniteCourante.equals(UniteLexicale.ecrire)) {
            ecriture();
        }

        else if (uniteCourante.equals(UniteLexicale.lire)) {
            lire();
        }

        else if (uniteCourante.equals(UniteLexicale.si)) {
            isAlternative();
        }

        else if (uniteCourante.equals(UniteLexicale.tantQue)) {
            isRepetitive();
        }
        ...
        else if (uniteCourante.equals(UniteLexicale.ident)) {
            affectation();
        }
    ...
    public void runAnalyseSyntaxique() throws SyntaxeException, SemantiqueExecption {
        tableSymbole = new ArrayList<Symbole>();
        uniteCourante = analyseurLexical.uniteSuivante();
        if (uniteCourante.equals(UniteLexicale.debutProgram)) {
            ...
            for (int i = 0; i < vic.size(); i++) {
                if (vic.get(i).getOperateur().equalsIgnoreCase("jzero")
                        || vic.get(i).getOperateur().equalsIgnoreCase("jump")) {
                    temp = table_etiq.get(j) + 1;
                    // System.out.println(" kikiki"+temp);
                    vic.get(i).setOperande("" + temp);
                    j++;
                }
            } //
            ...
            String str = "";
            for (InstruCible cible : vic)
                str += cible.getOperateur() + " " + cible.getOperande() + "\n";
            str += "end\n";
            Ecriture.write("src/main/resources/VIC.txt", str);
            vic.clear();
